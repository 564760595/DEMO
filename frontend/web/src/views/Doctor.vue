...
const sendMessage = async () => {
  ...
  try {
    const url = `http://127.0.0.1:9999/conversation/streamOllama?prompt=${encodeURIComponent(content)}${currentSessionId.value ? `&sessionId=${currentSessionId.value}` : ''}`
    ...
    const aiMsg = { sender: 'ai', content: '', timestamp: new Date().toLocaleTimeString() }
    ...
    while (true) {
      ...
      chunkCount++
      console.log(`读取第 ${chunkCount} 块: done=${done}, value=${value ? `长度=${value.length}` : null}`)
