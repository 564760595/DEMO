...
class KnowledgeInitializer {
    ...
    @PostConstruct
    void init() {
        CompletableFuture.runAsync(this::loadKnowledge);
    }
        ...
        try {
            ...
            for (Resource resource : knowledgeFiles) {
                try {
                    ...
                    log.warn("加载文件失败: {}, 原因: {}", resource.getFilename(), e.getMessage());
                }
            }

            if (!documents.isEmpty()) {
                TokenTextSplitter splitter = new TokenTextSplitter(512, 50, 100, 50, false);
